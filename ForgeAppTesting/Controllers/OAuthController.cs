...
namespace ForgeAppTesting.Controllers
{
    public class OAuthController : ControllerBase
    {
        ...
        [HttpGet]
        [Route("api/forge/oauth/token/files")]
        public async Task<dynamic> GetPublicTokenFilesAsync()
        {
            var scopes = new Scope[] { Scope.CodeAll, Scope.DataWrite, Scope.DataRead };
            var token = await Get2LeggedTokenAsync(scopes);
            return token;
        }
